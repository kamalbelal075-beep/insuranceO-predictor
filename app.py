...
st.caption("Cleans the insurance dataset, encodes it, trains selected regression models, and compares them.")
...
for model_name in model_options:
    if model_name == "XGBoost" and not XGB_AVAILABLE:
        st.sidebar.checkbox("XGBoost (not installed)", value=False, disabled=True)
    elif st.sidebar.checkbox(model_name, value=True):
...
@st.cache_resource
def train_models(x_train_scaled, y_train, selected_model_names):
    models = {}

    if "Linear Regression" in selected_model_names:
        models["Linear Regression"] = LinearRegression().fit(x_train_scaled, y_train)

    if "KNN" in selected_model_names:
    ...
    if "Gradient Boosting" in selected_model_names:
        models["Gradient Boosting"] = GradientBoostingRegressor().fit(
            x_train_scaled, y_train
        )

    if "Decision Tree" in selected_model_names:
        models["Decision Tree"] = DecisionTreeRegressor(
            max_depth=3, random_state=42
        ).fit(x_train_scaled, y_train)

    if "SVR" in selected_model_names:
        models["SVR"] = SVR(
            C=100, epsilon=0.01, gamma=0.1, kernel="rbf"
        ).fit(x_train_scaled, y_train)

    if "XGBoost" in selected_model_names and XGB_AVAILABLE:
        xg = XGBRegressor(
            n_estimators=100,
            learning_rate=0.15,
            max_depth=3,
...
st.sidebar.caption(f"{len(selected_models)} model(s) selected.")
